...
export default class EnumUtilities {
    ...
    static getEnumName<E, K extends string | number>(anEnum: { [key in K]: E }, aValue: string | number): string {
        const indexOf = Object.values(anEnum).indexOf(aValue as unknown as typeof anEnum);
        return Object.keys(anEnum)[indexOf];
    }

    @CatchAll((err) => console.trace(err))
    static getEnumKeyIterable<E, K extends string | number>(anEnum: { [key in K]: E }): string[] {
