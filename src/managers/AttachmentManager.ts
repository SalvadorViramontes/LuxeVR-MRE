...
export default class AttachmentManager extends MessageTrader {
	...
	public Update<User>(message: Message<User>): void {
		...
		if (message.type === "new-user") {
			this.NewUserAttachment(user.id);
		} else {
			return;
		}
	}
	...
	public async GetAllActorIds(userId: Guid): Promise<Guid[]> {
		...
		this.logHelper.info(
			"attachment-manager",
			`Getting all actor id attachments for user ${userId}`,
			LogVerbosity.Four
		);
	...
	public async NewUserAttachment(userId: Guid): Promise<void> {
		if (await this.userAttachmentMapper.has(userId)) {
			return;
		}

		this.logHelper.info(
			"attachment-manager",
			`New user attachment for user ${userId}`,
			LogVerbosity.Four
		);
	...
	public async SetUserAttachment(
		userId: Guid,
		...
		this.logHelper.info(
			"attachment-manager",
			`Setting actor ${actor.id} on ${attachmentPoint} for user ${userId}`,
			LogVerbosity.Four
		);
	...
	public async GetUserAttachment(
		...
		this.logHelper.info(
			"attachment-manager",
			`Getting attachment from ${attachmentPoint} for user ${userId}`,
			LogVerbosity.Four
		);
		const attachmentStruct = await this.userAttachmentMapper.get(userId);
	...
	public async RemoveUserAttachment(
		userId: Guid,
		attachmentPoint: AttachmentPoint
	): Promise<void> {
		if (!(await this.userAttachmentMapper.has(userId))) {
		...
		this.logHelper.info(
			"attachment-manager",
			`Removing attachment from ${attachmentPoint} for user ${userId}`,
			LogVerbosity.Four
		);
		const attachmentStruct = await this.userAttachmentMapper.get(userId);
	...
	public async RemoveAllUserAttachments(userId: Guid): Promise<void> {
		...
		this.logHelper.info(
			"attachment-manager",
			`Removing all actor attachments for user ${userId}`,
			LogVerbosity.Four
		);
		const attachmentStruct = await this.userAttachmentMapper.get(userId);
